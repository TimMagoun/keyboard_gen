...
from solid.utils import right, back, forward, down, left
...
class Body:
    ...
    #     self.min_y = min_y
    #     self.max_y = max_y
    #     # self.logger.debug('min_x: %f, max_x: %f, max_y: %f, min_y: %f', self.min_x,
    #      self.max_x, self.max_y, self.min_y)
        ...
        if self.plate_supports:
            ...
            for x in range(max_x_ceil):
                ...
                for y in range(max_y_ceil):
                    ...
                    # x_offset = x - ((self.real_max_x / 2) + (self.side_margin_diff / 2))
                    # self.parameters.switch_spacing
                    # y_offset = -(y - ((self.real_max_y / 2) + (self.top_margin_diff / 2)) /
                    # self.parameters.switch_spacing)
                    x_offset = x
                    y_offset = -y
    ...
    def screw_hole(self, tap=False):
        try:
            ...
            self.logger.error("Error getting screw hole radius: %s", str(e))
            return None
    ...
    def generate_screw_holes_coordinates(self):
        ...
        # self.logger.debug('self.real_case_width: %f, self.screw_edge_inset: %f, self.screw_diameter: %f'
        # , self.real_case_width, self.screw_edge_inset, self.screw_diameter)
        # self.logger.debug('self.real_case_width - ((self.screw_edge_inset * 2) + self.screw_diameter): %f'
        # , self.real_case_width - ((self.screw_edge_inset * 2) + self.screw_diameter))
    ...
    def screw_hole_objects(self, tap=False):
        ...
        for coord_string in self.screw_hole_info.keys():
            ...
            if (
                self.parameters.cable_hole
                and y == self.y_screw_width
                and x == self.x_screw_width / 2
            ):
        ...
        # self.logger.debug('-self.left_margin: %f, self.screw_edge_inset: %f, x_offset: %f', -self.left_margin,
        # self.screw_edge_inset, x_offset)
