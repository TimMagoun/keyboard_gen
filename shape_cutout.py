from solid import circle, polygon, linear_extrude, square
...
class ShapeCutout(Cell):
    ...
    def __init__(
        self, x, y, shape_type, shape_parameters, parameters: Parameters = Parameters()
    ):
    ...
    def rectangle_cutout(self):
        ...
        if (
            "width" in self.shape_parameters.keys()
            and "height" in self.shape_parameters.keys()
        ):
            ...
            self.logger.error(
                'At least a "width" or "height" must be provided for a custom rectangle cutout. Specifying only 1'
                " of those will create a square"
            )
            exit(1)
