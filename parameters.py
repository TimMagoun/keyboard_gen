...
class Parameters:
    ...
    def __init__(self, parameter_dict: dict = None):
        ...
        self.paramater_alternate_dict = {"plate_wall_thickness": "case_wall_thickness"}
    ...
    def build_attr_from_dict(self, parameter_dict):
        ...
        for param in parameter_dict.keys():
            ...
            if param in self.paramater_alternate_dict.keys():
                ...
                # If the new version of the parameter is not in the paramter dict then us the value in the
                # deprectaed parameter
            ...
            if param == "custom_switch":
                ...
                if "path" in value.keys():
                    ...
                    self.logger.warning(
                        'Custom Switch defined but no "path" list defined. Points in "ponts"'
                        " list will be used in defined order"
                    )
    ...
    def validate_parameters(self):
        ...
        #     error_message += 'Screw Edge Inset %f must be greater than case_wall_thickness: %f +
        # screw_hole_body_radius: %f = %f\n' % (self.screw_edge_inset, self.case_wall_thickness,
        # self.screw_hole_body_radius, self.case_wall_thickness + self.screw_hole_body_radius)
        ...
        if self.switch_type not in self.switch_config.switch_type_function_dict.keys():
            parameter_error = True
            error_message += "switch type %s is not a valid switch type" % (
                self.switch_type
            )

        if (
            self.stabilizer_type
            not in self.switch_config.stab_type_function_dict.keys()
        ):
            parameter_error = True
            error_message += "stabilizer type %s is not a valid stabilizer type" % (
                self.stabilizer_type
            )
